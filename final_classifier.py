...
class CLASSIFIER:
    ...
    def val_gzsl_class(self, test_X, test_label, target_classes):
        with torch.no_grad():
            start = 0
            ntest = test_X.size()[0]
            predicted_label = torch.LongTensor(test_label.size())
            ...
            #print(str(predicted_label[:3]).ljust(40,'.'), end= ' '     )
            acc = self.compute_per_class_acc_gzsl_wo_mean(test_label, predicted_label, target_classes)
            print(' ')
            for i in range(acc.size()[0]):
                print('Class {:d}: {:.1f}'.format(target_classes[i].detach().item(), acc[i].detach().item()*100))
    ...
    def compute_per_class_acc_gzsl_wo_mean(self, test_label, predicted_label, target_classes):
